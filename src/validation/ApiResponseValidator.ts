...
export class ApiResponseValidator {
  ...
  private validateResponseProp(apiResponse: ApiResponse): SearchError | undefined {
    if (!apiResponse.response) {
      return new SearchError('Malformed Answers API response: missing response property.');
    }
  }

  private validateMetaProp(apiResponse: ApiResponse): SearchError | undefined {
    if (!apiResponse.meta) {
      return new SearchError('Malformed Answers API response: missing meta property.');
    }
  }
